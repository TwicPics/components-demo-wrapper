...
export const SlideshowItem = ( { children, width } ) => (
    <div className={styles[ `slideshow-item` ]} style={ {
        width,
    } }>
        { children }
    </div>
...
const Slideshow = ( { children } ) => {
    ...
    return (
        <div className={styles[ `slideshow-container` ]}>
            <button className={ `${ styles[ `slick-prev` ] } ${ styles[ `slick-arrow` ] }`} onClick={ () => {
                updateIndex( activeIndex - 1 );
            } }>&#8249;</button>
            ...
            <button className={ `${ styles[ `slick-next` ] } ${ styles[ `slick-arrow` ] }`} onClick={ () => {
                updateIndex( activeIndex + 1 );
            } }>&#8250;</button>
        </div>
    );
